...
class SupportTicketPriority extends Notification
{
    /**
    ...
    protected function buildMailMessage(string $url, SupportTicket $notifiable): MailMessage
    {
        $message = (new MailMessage())
            ->subject('[Ticket #' . $notifiable->id . '] ' . Lang::get('Priority changed to') . ' "' . __(ucfirst($notifiable->priority)) . '"')
            ->line(Lang::get('The ticket priority has changed to "' . ucfirst($notifiable->priority) . '".'))
