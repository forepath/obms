...
class OrderNewPendingApproval extends Notification
{
    /**
    ...
    protected function buildMailMessage(string $url, ShopOrderQueue $notifiable): MailMessage
    {
        $message = (new MailMessage())
            ->subject(Lang::get('New order placed: ') . $notifiable->number)
            ->line(Lang::get('Your order :id has been placed successfully. Before it can be processed it needs to be approved by an employee. This might take a while. You will be informed when the order status changes.', [
